...
while 1: # Запускаем общий цикл для всего - оптимизировать?
    ...
    def distance():
        ...
        if GPIO.input(GPIO_ECHO) == 0:  # пока на эхо значение 0 записываем значение времени
            StartTime = time.time()

        # сохраняем время возвращения
        if GPIO.input(GPIO_ECHO) == 1:  # пока на эхо значение 1 (возврат сигнала) записываем значение времени
            StopTime = time.time()
