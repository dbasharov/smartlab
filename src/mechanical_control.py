...
while 1: # Запускаем общий цикл для всего - оптимизировать?
    ...
    if flLeft:
        ...
        FL_on_B = 4096 # переднее левое крутим назад
        FR_on_F = 4096 # переднее правое крутим вперед
        BL_on_B = 4096 # заднее левое крутим назад
        BR_on_F = 4096 # заднее правое крутим вперед
...
#
    # правый танковый разворот
        ...
        x += speed
        FL_on_F = 4096 # переднее левое крутим вперед
        FR_on_B = 4096 # переднее правое крутим назад
        BL_on_F = 4096 # заднее правое крутим вперед
        BR_on_B = 4096 # заднее правое крутим назад
    ...
    # все вперед
        ...
        FL_on_F = 4096
        FR_on_F = 4096 # переднее правое колесо вперед - значение ШИМ
        BL_on_F = 4096
        BR_on_F = 4096
    ...
    # все назад
        ...
        y += speed
        FL_on_B = 4096 
        FR_on_B = 0 # переднее правое колесо назад - значение ШИМ - можно всегда 0 - начало периода, т.е. с самого начала периода "поднимаем" уровень
        FR_off_B = 3072 # переднее правое колесо назад - значение ШИМ - нужное значение (в данном случае 3072 = 75%), определяет скорость вращения двигателя, в нужном месте "отпускает уровень", максимальное значение 4095 - значит, что уровень поднят постоянно, фактически не ШИМ, а прямая
        BL_on_B = 4096
        BR_on_B = 4096
    ...
    print ("FR_on_B=", FR_on_B)
    print ("FR_off_B=", FR_off_B)
